...
/* ---- inlined: components/fragrance-performance-meter/styles.ts ---- */
...
/* ---- inlined: components/fragrance-performance-meter/types.ts ---- */
type MeterStyle = 'bars' | 'rings';
...
type FixedMetricKey = (typeof FIXED_METRIC_KEYS)[number];

/* ---- inlined: components/fragrance-performance-meter/utils.ts ---- */
...
function resolveMeterStyle(config: Record<string, unknown>): MeterStyle {
...
function parseMetrics(config: Record<string, unknown>): PerformanceMetric[] {
...
function hasMetrics(metrics: PerformanceMetric[]): boolean {
  return metrics.length > 0;
}

function ringDashOffset(value: number, radius = 42): string {
  const circumference = 2 * Math.PI * radius;
  const filled = (value / 100) * circumference;
  return String(circumference - filled);
}

function ringCircumference(radius = 42): number {
  return 2 * Math.PI * radius;
}

/* ---- inlined: components/fragrance-performance-meter/index.ts ---- */
