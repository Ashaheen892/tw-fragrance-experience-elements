...
/* ---- inlined: components/scent-passport/styles.ts ---- */
...
/* ---- inlined: components/scent-passport/types.ts ---- */
...
type PassportStepKey = 'liked' | 'disliked' | 'strength' | 'times' | 'character';

interface PassportNavLabels {
  next: string;
  back: string;
  see: string;
  reset: string;
...
/* ---- inlined: components/scent-passport/utils.ts ---- */
...
function parseNotes(raw: unknown): NoteOption[] {
...
function parseStrengths(raw: unknown): StrengthOption[] {
  return normalizeCollection(raw)
    .map((item, i) => {
...
function parseTimes(raw: unknown): TimeOption[] {
  return normalizeCollection(raw)
    .map((item, i) => ({
...
function parseCharacters(raw: unknown): CharacterOption[] {
  return normalizeCollection(raw)
    .map((item, i) => ({
...
function parsePassportResults(raw: unknown): PassportResult[] {
  return normalizeCollection(raw)
    .map((item, i) => ({
...
function parseNavLabels(config: Record<string, unknown>): PassportNavLabels {
  return {
...
function emptySelections(): PassportSelections {
  return {
    likedIds: [],
    dislikedIds: [],
    strengthId: '',
...
/* ---- inlined: components/scent-passport/index.ts ---- */
