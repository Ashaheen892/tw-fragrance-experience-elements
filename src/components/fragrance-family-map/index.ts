...
/* ---- inlined: components/fragrance-family-map/styles.ts ---- */
...
/* ---- inlined: components/fragrance-family-map/types.ts ---- */
type FamilyMapLayout = 'wheel' | 'grid';

interface FragranceFamilyItem {
  id: string;
  name: string;
  desc: string;
  color: string;
...
/* ---- inlined: components/fragrance-family-map/utils.ts ---- */
const LAYOUTS: FamilyMapLayout[] = ['wheel', 'grid'];

function parseFamilies(raw: unknown): FragranceFamilyItem[] {
...
function resolveLayout(config: Record<string, unknown>): FamilyMapLayout {
  const value = getRadioValue(config.ffm_layout, 'grid') as FamilyMapLayout;
  return LAYOUTS.includes(value) ? value : 'grid';
}

/* ---- inlined: components/fragrance-family-map/index.ts ---- */
